...
void minimize_curvature(SurfaceMesh& mesh, const LaplaceConfig config)
{
    fair(mesh, config, 2);
}
...
public:
    ...
    void fill_hole(Halfedge h, const LaplaceConfig config);
...
void HoleFilling::fill_hole(Halfedge h, const LaplaceConfig config)
{
    if (!h.is_valid())
    {
    ...
    try
    {
        ...
        refine(config);            // refine filled-in edges
    }
...
void HoleFilling::refine(const LaplaceConfig config)
{
    const int n = hole_.size();
    Scalar l, lmin, lmax;
...
void HoleFilling::fairing(const LaplaceConfig config)
{
    ...
    {
        // fair new vertices
        minimize_curvature(mesh_, config);
    }
...
void fill_hole(SurfaceMesh& mesh, Halfedge h, const LaplaceConfig config)
{
    HoleFilling(mesh).fill_hole(h, config);
}
